...
image: /images/blog/buttrfly_effect.jpg
categories:
  - R
...
On my [last post](https://iyarlin.github.io/blog/correlation-is-not-causation-so-what-is/) I gave an intuitive demonstration of what's causal inference and how it's different than classic ML.
...
$\qquad PD(x)=\mathbb{E}(Y|X=x)=\mathbb{E}_Z(f(x,Z))$
...
$\qquad \hat{PD}(x) = \frac{1}{n}\sum_{i=1}^{n} f(x,z_i)$
...
To demonstrate why this is a misconception let's briefly revisit the toy model introduced on my [last post](https://iyarlin.github.io/blog/correlation-is-not-causation-so-what-is/):
...
In the case where we can measure $Z_B$ the following equality holds:

\begin{equation} 
\mathbb{E}(Y|do(X=x)) = \mathbb{E}_{Z_B}(f(x,Z_B))
\end{equation}

where the right hand side can be estimated from samples $\{y_i,x_i,{z_{B}}_i\}$ generated by $DGP_{\text{obs}}$ using: 

$\qquad \hat{\mathbb{E}}_{Z_B}(f(x,Z_B)) = \frac{1}{n}\sum_{i=1}^{n} f(x,z_{B_i})$
...
For a proof of quation 5 read chapter 3 of Pearls' light intro to causal inference [Causal Inference in Statistics - A Primer](http://bayes.cs.ucla.edu/PRIMER/). 
